...
export default function KidsGames({ user, onClose, addStar }) {
  const { t } = useApp();
  ...
  const finish = async (finalScore) => {
    ...
    if (finalScore === TOTAL_Q && addStar) { try { addStar(1, t("kg_perfectScore")); } catch {} }
  };

  const GAMES = [
    { id: "math", t: "Matematik Ninja", d: t("kg_mathDesc"), e: "🥷", g: "linear-gradient(135deg,#7c3aed,#a855f7)", art: "➕✖️", on: () => setView("math-lvl") },
    { id: "word", t: "So'z Ustasi", d: t("kg_wordDesc"), e: "🇬🇧", g: "linear-gradient(135deg,#ea580c,#f59e0b)", art: "🔤", on: () => startGame("word") },
    { id: "geo",  t: "Geografiya", d: t("kg_geoDesc"), e: "🌍", g: "linear-gradient(135deg,#0e7490,#06b6d4)", art: "🗺️", soon: true },
    { id: "sci",  t: "Tabiat fanlari", d: t("kg_sciDesc"), e: "🔬", g: "linear-gradient(135deg,#15803d,#22c55e)", art: "🧪", soon: true },
  ];
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 70, maxWidth: 480, margin: "0 auto", background: "#0d0d12", overflowY: "auto", fontFamily: "'Segoe UI',system-ui,sans-serif" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", padding: "16px 16px 8px", position: "sticky", top: 0, background: "#0d0d12ee", backdropFilter: "blur(8px)", zIndex: 5 }}>
        ...
        <div style={{ flex: 1, textAlign: "center", fontSize: 19, fontWeight: 800, color: "#fff", letterSpacing: 1 }}>{t("kg_gamesTitle")}</div>
        <div style={{ width: 40 }} />
      </div>
          ...
          {GAMES.map(g => (
            <div key={g.id} onClick={() => !g.soon && g.on()} style={{ background: g.g, borderRadius: 22, padding: "18px 18px", marginBottom: 14, cursor: g.soon ? "default" : "pointer", position: "relative", overflow: "hidden", opacity: g.soon ? 0.65 : 1, animation: "kgPop .3s ease" }}>
              ...
              <div style={{ marginTop: 10, display: "flex", gap: 8, alignItems: "center" }}>
                {g.soon
                  ? <span style={{ fontSize: 10.5, fontWeight: 800, background: "rgba(0,0,0,.3)", color: "#fff", borderRadius: 10, padding: "4px 10px" }}>{t("kg_soon")}</span>
                  : <>
                      <span style={{ fontSize: 11, fontWeight: 800, background: "rgba(255,255,255,.22)", color: "#fff", borderRadius: 10, padding: "4px 12px" }}>▶ {t("kg_play")}</span>
                      {(g.id === "math" ? Math.max(bestOf("math1"), bestOf("math2"), bestOf("math3")) : bestOf("word")) > 0 &&
                        <span style={{ fontSize: 10.5, color: "rgba(255,255,255,.85)", fontWeight: 700 }}>🏅 {t("kg_record")}: {g.id === "math" ? Math.max(bestOf("math1"), bestOf("math2"), bestOf("math3")) : bestOf("word")}/{TOTAL_Q}</span>}
                    </>}
              </div>
            </div>
          ))}
      ...
      {view === "math-lvl" && (
        <div style={{ padding: "20px 16px", animation: "kgPop .25s ease" }}>
          ...
          <div style={{ textAlign: "center", fontSize: 18, fontWeight: 800, color: "#fff", marginBottom: 18 }}>{t("kg_chooseLevel")}</div>
          {[
            { l: 1, t: t("kg_easy"), d: "+ − (20 gacha)", c: "#22c55e" },
            { l: 2, t: t("kg_medium"), d: "+ − × (50 gacha)", c: "#f59e0b" },
            { l: 3, t: t("kg_hard"), d: "+ − × ÷ (100 gacha)", c: "#ef4444" },
          ].map(x => (
        ...
        <div style={{ padding: "12px 16px 40px" }}>
          ...
          <div key={qNum} style={{ background: "#1c1c26", borderRadius: 22, padding: "34px 20px", textAlign: "center", marginBottom: 20, animation: "kgPop .25s ease" }}>
            <div style={{ fontSize: 11, color: "#8b8b9e", fontWeight: 700, marginBottom: 10 }}>
              {game === "math" ? t("kg_calculate") : t("kg_howInEnglish")}
            </div>
      ...
      {view === "result" && (
        <div style={{ padding: "30px 20px", textAlign: "center", animation: "kgPop .3s ease" }}>
          ...
          <div style={{ fontSize: 22, fontWeight: 800, color: "#fff", marginBottom: 6 }}>
            {score >= 8 ? t("kg_greatResult") : score >= 5 ? t("kg_good") : t("kg_tryAgain")}
          </div>
          ...
          {score >= 5 && <div style={{ fontSize: 12.5, color: "#22c55e", fontWeight: 700, marginBottom: 4 }}>+{score >= 8 ? 5 : 2} {t("kg_leaderboardPoints")}</div>}
          {score === TOTAL_Q && <div style={{ fontSize: 12.5, color: "#f59e0b", fontWeight: 700 }}>⭐ +1 {t("kg_starForPerfect")}</div>}
          <div style={{ display: "flex", gap: 12, marginTop: 26 }}>
            <button onClick={() => setView("hub")} style={{ ...btn, flex: 1, background: "#1c1c26", borderRadius: 16, padding: 15, color: "#8b8b9e", fontWeight: 800, fontSize: 14 }}>{t("kg_gamesButton")}</button>
            <button onClick={() => game === "math" ? startGame("math", lvl) : startGame("word")} style={{ ...btn, flex: 1.4, background: "linear-gradient(135deg,#8b5cf6,#6366f1)", borderRadius: 16, padding: 15, color: "#fff", fontWeight: 800, fontSize: 14 }}>🔄 {t("kg_playAgain")}</button>
          </div>
        </div>
      )}
    </div>
