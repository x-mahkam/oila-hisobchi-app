        ...
        <div style={{ padding: "6px 16px 50px", animation: "wcPop .25s ease" }}>
          ...
          <AppCard th={th} style={{ border: "1.5px solid " + CHART[3] + ALPHA.strong, marginBottom: SPACE.s3 }}>
            <div style={{ ...TYPE.subtitle, fontWeight: 800, color: th.t1, display: "flex", alignItems: "center", gap: SPACE.s2, marginBottom: SPACE.s3 }}>
              {WIco.save(CHART[3], 16)}{editingId ? L("Hisobni yangilash", "Обновить смету") : L("Hisobni saqlash", "Сохранить смету")}
            </div>
            ...
            <PrimaryButton th={th} onClick={saveWedding} style={{ marginBottom: 0 }}>{WIco.save("#fff", 15)}{editingId ? L("Yangilash", "Обновить") : L("Saqla", "Сохранить")}</PrimaryButton>
          </AppCard>
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
            ...
            <button onClick={() => { if (confirm(L("Smeta o'chirilib, qaytadan boshlansinmi?", "Начать заново?"))) { setData(false); setStep("side"); setWEv([]); setEditingId(null); setWName(""); setWStatus("reja"); setScreen("wizard"); db.s("toy_" + oilaId, { saved: savedRef.current, upd: Date.now() }).catch(() => {}); } }}
