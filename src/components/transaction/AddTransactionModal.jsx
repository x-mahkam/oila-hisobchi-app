...
export default function AddTransactionModal({
  show, onClose,
  tab, setTab, step, setStep, kat, setKat,
  // Xarajat
  fS, setFS, fIz, setFIz, fSn, setFSn, fRp, setFRp,
  fK, setFK, xForMember, setXForMember, xMode, setXMode,
  // Daromad
  fDS, setFDS, fDT, setFDT, fDI, setFDI,
  // Handlers
  addX, addD,
  // Context
  user, azolar, isKid, th, lg, isPremium,
  startScanner, startVoice, S, Ico, f, td
}) {
  if (!show) return null;
  return (
<div style={{position:"fixed",inset:0,zIndex:500,display:"flex",flexDirection:"column",animation:"slideUpFull .3s cubic-bezier(.32,1.4,.64,1)"}}>
      ...
      <div style={{background:th.sur,borderBottom:"1px solid "+th.bor,padding:"14px 18px 12px",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
        <button onClick={()=>{if(step==="form"){setStep("kat");}else{onClose();}}} style={{background:"none",border:"none",fontSize:15,color:th.t2,cursor:"pointer",fontWeight:600,padding:"4px 0"}}>
          {step==="form"?"← "+lg==="uz"?"Orqaga":"Back":"Bekor"}
        </button>
      ...
      {/* Tabs */}
      {step==="kat"&&!isKid&&<div style={{background:th.sur,padding:"10px 16px 12px",flexShrink:0,borderBottom:"1px solid "+th.bor}}>
      ...
      <div style={{flex:1,overflowY:"auto",background:th.bg}}>
        {/* KATEGORIYA TANLASH */}
        {step==="kat"&&tab==="xarajat"&&!isKid&&<div style={{padding:"14px 16px"}}>
          {/* QR + Ovoz */}
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
            <button onClick={()=>{onClose();startScanner();}} style={{background:"linear-gradient(135deg,"+th.ac+","+th.ac2+")",border:"none",borderRadius:16,padding:"14px 10px",color:"#fff",fontSize:13,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:6,boxShadow:"0 4px 14px "+th.ac+"44",position:"relative"}}>
            ...
            <button onClick={()=>{onClose();startVoice();}} style={{background:"linear-gradient(135deg,#8b5cf6,#6366f1)",border:"none",borderRadius:16,padding:"14px 10px",color:"#fff",fontSize:13,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:6,boxShadow:"0 4px 14px #8b5cf644",position:"relative"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8}}>
            {KATS.map((k,i)=>(
              <button key={k.id} onClick={()=>{setKat(k.id);setFK(k.id);setStep("form");}} style={{background:th.sur,border:"1.5px solid "+th.bor,borderRadius:16,padding:"12px 6px 10px",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:6,transition:"all .15s"}}>
        ...
        {/* DAROMAD KATEGORIYA */}
        {step==="kat"&&tab==="daromad"&&<div style={{padding:"14px 16px"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8}}>
            {DARS.map((d,i)=>(
              <button key={d.id} onClick={()=>{setKat(d.id);setFDT(d.id);setStep("form");}} style={{background:th.sur,border:"1.5px solid "+th.bor,borderRadius:16,padding:"12px 6px 10px",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:6}}>
        ...
        {/* XARAJAT FORM */}
        {step==="form"&&tab==="xarajat"&&<div style={{padding:"16px"}}>
          ...
          <button onClick={async()=>{await addX();onClose();}} style={{...S.bt(th.rd,"#dc2626"),marginBottom:8}}>
            {Ico.check("#fff")}{lg==="uz"?" Xarajatni saqlash":" Save expense"}
          </button>
        </div>}
        {/* DAROMAD FORM */}
        {step==="form"&&tab==="daromad"&&<div style={{padding:"16px"}}>
          ...
          <button onClick={async()=>{await addD();onClose();}} style={{...S.bt(),marginBottom:8}}>
