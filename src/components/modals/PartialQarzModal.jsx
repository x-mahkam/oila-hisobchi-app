...
export default function PartialQarzModal({ q, partialSum, setPartialSum, th, STY, f, t, onConfirm, onClose }) {
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.6)", zIndex: 1001, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }} onClick={onClose}>
      <div style={{ background: th.sur, borderRadius: 24, padding: "26px 24px", width: "100%", maxWidth: 360 }} onClick={e => e.stopPropagation()}>
        <div style={{ textAlign: "center", marginBottom: 18 }}>
          ...
          <div style={{ fontSize: 17, fontWeight: 800, color: th.t1, marginBottom: 4 }}>{t("pqm_title")}</div>
          <div style={{ fontSize: 13, color: th.t2 }}>{q.kim} · {t("pqm_total")}: {f(q.summa, true)}</div>
        </div>
        <label style={STY.lb}>{t("pqm_returnedAmount")}</label>
        ...
        <div style={{ display: "flex", gap: 6, marginBottom: 14, flexWrap: "wrap" }}>
          ...
          <button onClick={() => setPartialSum(String(q.summa))} style={{ flex: 1, background: th.ac + "15", border: "1px solid " + th.ac + "33", borderRadius: 9, padding: "7px 0", color: th.ac, cursor: "pointer", fontSize: 12, fontWeight: 600 }}>{t("pqm_all")}</button>
        </div>
        {pay > 0 && (
          <div style={{ background: th.bg, borderRadius: 12, padding: "12px 14px", marginBottom: 16 }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
              <span style={{ fontSize: 12, color: th.t2 }}>{t("pqm_paying")}</span>
              <span style={{ fontSize: 13, fontWeight: 700, color: dc }}>{f(pay, true)}</span>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              <span style={{ fontSize: 12, color: th.t2 }}>{t("pqm_remaining")}</span>
              <span style={{ fontSize: 13, fontWeight: 700, color: remain <= 0 ? th.gr : th.t1 }}>{remain <= 0 ? t("pqm_fullyClosed") : f(remain, true)}</span>
            </div>
          </div>
        )}
        <div style={{ display: "flex", gap: 10 }}>
          ...
          <button onClick={onConfirm} style={{ flex: 2, background: "linear-gradient(135deg," + dc + "," + dc + "cc)", border: "none", borderRadius: 14, padding: "13px", color: "#fff", cursor: "pointer", fontWeight: 700, fontSize: 14 }}>{t("pqm_confirm")}</button>
        </div>
      </div>
    </div>
  );
