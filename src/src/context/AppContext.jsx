...
export function AppProvider({ children }) {
  ...
  // sezilarli kech bo'lardi. Boshlang'ich qiymatlar endi mahalliy keshdan
  // (tarmoqsiz, darhol) olinadi; agar kesh bo'lmasa (juda birinchi marta)
  // tarmoqqa tushiladi. Yozuvlar esa fonda (kutmasdan) davom etadi.
  const addStar = useCallback(async (count = 1, reason = "") => {
    if (!user?.oilaId) return;
    try {
      const cachedStars = db.gCache("stars_" + user.oilaId);
      const cur = Math.max(0, (cachedStars != null ? cachedStars : await db.g("stars_" + user.oilaId)) || 0);
      const next = Math.max(0, cur + count);
      setStars(next);
      db.s("stars_" + user.oilaId, next).catch(() => {});
      ...
      const cachedCoins = db.gCache("baraka_coins_" + user.oilaId);
      const curC = (cachedCoins != null ? cachedCoins : await db.g("baraka_coins_" + user.oilaId)) || 0;
      db.s("baraka_coins_" + user.oilaId, curC + coinAmt).catch(() => {});
