...
export default function SavingsSequenceGame({ user, lg = "uz", dark, gameId = "logic/pattern", name = "", level, onBack, onNextLevel }) {
  ...
  const eng = useGameEngine({
    questionCount: level ? level.questionCount : 10,
  ...
  useEffect(() => {
    if (eng.phase === "result" && eng.result && !hasSavedRef.current) {
      ...
      const stars = level ? starsFor(pct, level.passPct) : (r.correct >= 9 ? 3 : r.correct >= 7 ? 2 : r.correct >= 5 ? 1 : 0);
  ...
  if (eng.phase === "result" && eng.result) {
    const r = eng.result;
    const pct = Math.round((r.correct / r.total) * 100);
    const finalStars = level ? starsFor(pct, level.passPct) : (r.correct >= 9 ? 3 : r.correct >= 7 ? 2 : r.correct >= 5 ? 1 : 0);
