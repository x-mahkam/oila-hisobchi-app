...
import { Av, MoneyInput } from "../components/common/index.jsx";
    ...
    <div>
      <div style={{ ...S.row, marginBottom: 16 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <button onClick={() => setShowAddQarz(v => !v)} style={{ background: th.ac, border: "none", borderRadius: 10, padding: "7px 14px", color: "#fff", cursor: "pointer", fontWeight: 700, fontSize: 13, boxShadow: "0 4px 12px " + th.ac + "44" }}>{showAddQarz ? "×" : (lg === "uz" ? "+ Qo'shish" : "+ Add")}</button>
        </div>
      </div>
      ...
      {qarzReqs.length > 0 && (
        <div style={{ ...S.cd, border: "1.5px solid " + th.am + "55", marginBottom: 14, background: th.am + "0a" }}>
          ...
          {qarzReqs.map(req => {
            ...
            return (
              <div key={req.id} style={{ background: th.sur, borderRadius: 13, padding: "13px 15px", marginBottom: 10, border: "1px solid " + (isPay ? th.gr + "44" : th.bor) }}>
              ...
              <div key={q.id} style={{ ...S.cd, padding: "13px 15px", marginBottom: 10, border: "1px solid " + (overdue ? th.rd + "55" : th.bor) }}>
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 10 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    <div>
                      <div style={{ fontWeight: 700, fontSize: 15, color: th.t1, display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                        {q.kim}
                  ...
                  <div style={{ textAlign: "right" }}>
                    <div style={{ fontSize: 16, fontWeight: 800, color }}>{f(q.summa, true)}</div>
                    {q.paidPart > 0
                      ? <div style={{ fontSize: 9, color: th.gr, marginTop: 2, fontWeight: 600 }}>{lg === "uz" ? "To'langan: " : "Paid: "}{f(q.paidPart, true)}</div>
                      : <div style={{ fontSize: 10, color: th.t2, marginTop: 2 }}>{isLent ? (lg === "uz" ? "ular qarzli" : "they owe") : (lg === "uz" ? "men qarzman" : "I owe")}</div>}
                  </div>
                </div>
                <div style={{ display: "flex", gap: 8 }}>
                  {q.payStatus === "pending"
                    ...
                    : <button onClick={() => markQarzPaid(q.id)} style={{ flex: 1, background: color + "18", border: "1px solid " + color + "44", borderRadius: 10, padding: "8px 0", color, cursor: "pointer", fontWeight: 700, fontSize: 12 }}>{isLent ? (lg === "uz" ? "Qaytarib oldim" : "Got it back") : (lg === "uz" ? "Qaytardim" : "Paid back")}</button>}
                  {q.payStatus !== "pending" && isLent && (
                    <button onClick={() => sendQarzReminder(q)} style={{ background: (overdue ? th.rd : th.am) + "15", border: "1px solid " + (overdue ? th.rd : th.am) + "44", borderRadius: 10, padding: "8px 11px", color: overdue ? th.rd : th.am, cursor: "pointer", fontWeight: 700, fontSize: 12, flexShrink: 0, display: "flex", alignItems: "center", gap: 4 }}>
                      <svg width="14" height="14" viewBox="0 0 18 18" fill="none"><path d="M9 2a5 5 0 00-5 5v3l-1.5 2.5h13L14 10V7a5 5 0 00-5-5z" stroke={overdue ? th.rd : th.am} strokeWidth="1.4" strokeLinejoin="round" /></svg>
                      {lg === "uz" ? "Eslatish" : "Remind"}
                    </button>
                  )}
                  {q.payStatus !== "pending" && (
                    <button onClick={() => { setPartialQarz(q); setPartialSum(""); }} style={{ background: th.ac + "11", border: "1px solid " + th.ac + "33", borderRadius: 10, padding: "8px 12px", color: th.ac, cursor: "pointer", fontWeight: 700, fontSize: 12, flexShrink: 0 }}>{lg === "uz" ? "Qisman" : "Partial"}</button>
                  )}
                  {q.linked && q.linkStatus === "accepted" && q.payStatus !== "pending" && (
                    <button onClick={() => generateTilxat(q)} style={{ background: "#4f46e515", border: "1px solid #4f46e544", borderRadius: 10, padding: "8px 11px", color: "#4f46e5", cursor: "pointer", fontWeight: 700, fontSize: 12, flexShrink: 0, display: "flex", alignItems: "center", gap: 4 }} title={lg === "uz" ? "Tilxat (PDF)" : "Receipt (PDF)"}>
            ...
            return (
              <div key={q.id} style={{ ...S.cd, padding: "12px 14px", marginBottom: 9, border: "1px solid " + th.bor, borderLeft: "3px solid " + th.ac }}>
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  ...
                  <div style={{ textAlign: "right" }}>
                    <div style={{ fontSize: 15, fontWeight: 800, color }}>{f(q.summa, true)}</div>
                    <div style={{ fontSize: 10, color: th.t2, marginTop: 2 }}>{isLent ? (lg === "uz" ? "ular qarzli" : "they owe") : (lg === "uz" ? "qarzdor" : "owes")}</div>
                  </div>
                </div>
              </div>
            );
              ...
              <div key={q.id} style={{ ...S.cd, padding: "11px 14px", marginBottom: 8, borderLeft: "3px solid " + dc + "66" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    <div>
                      <div style={{ fontWeight: 600, fontSize: 13, color: th.t1 }}>{q.kim}</div>
                      <div style={{ fontSize: 10, color: dc, fontWeight: 600 }}>{isLent ? (lg === "uz" ? "Qaytarib oldim" : "Got back") : (lg === "uz" ? "Qaytardim" : "Paid back")} · {q.paidSana}</div>
                    </div>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <span style={{ fontSize: 13, fontWeight: 800, color: dc }}>{isLent ? "+" : "−"}{f(q.summa, true)}</span>
                    <button onClick={() => delQarz(q.id)} style={{ background: "none", border: "none", cursor: "pointer" }}>{Ico.trash(th.t2)}</button>
                  </div>
                </div>
              </div>
            );
      ...
      {qarzlar.length === 0 && !showAddQarz && qarzReqs.length === 0 && (
        <div style={{ textAlign: "center", padding: "44px 0", color: th.t2, display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
          ...
          <div style={{ fontSize: 15 }}>{lg === "uz" ? "Hali qarz yo'q" : "No debts yet"}</div>
        </div>
      )}
    </div>
  );
